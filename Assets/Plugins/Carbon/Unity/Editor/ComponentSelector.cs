...
{
	/// <summary>
	/// ComponentSelector (modified from NGUI)
	/// </summary>
	public class ComponentSelector : ScriptableWizard
	{
		...
		static public void Draw<T>(string buttonName, T obj, OnSelectionCallback cb, bool editButton, params GUILayoutOption[] options) where T : Object
		{
			...
			bool show = EditorUtils.DrawPrefixButton(buttonName);
		...
		{
			Draw<T>(typeof(T).Name, obj, cb, editButton, options);
		}

		/// <summary>
		/// Show the selection wizard.
		/// </summary>
		public static void Show<T>(OnSelectionCallback cb) where T : Object
		{
		...
		public static void Show<T>(OnSelectionCallback cb, string[] extensions) where T : Object
		{
			...
			string title = string.Format("Select a {0}", type.Name);
