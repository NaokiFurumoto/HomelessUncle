...
public static partial class EditorHelp
{
    static EditorHelp()
    {
        //FlowNode_Comment.CommentHelpers.Add(DrawCodeJumpButton);
    }

    static void DrawCodeJumpButton(string comment)
    {
        // e.g.) [EditorHelp.cs]
        // e.g.) [EditorHelp.cs:32]
        // e.g.) [EditorHelp.cs#DrawCodeJumpButton]
        ...
        var j = -1;
        var k = -1;
        if ((j = comment.IndexOf(".cs]", i)) > -1)
        {
            if (GUILayout.Button("Open CS In Editor"))
            {
                var name = comment.Substring(i, j - i);
                var path = FindAssetPath(name, " t:Script");
                if (string.IsNullOrEmpty(path))
                    return;
                UnityEditorInternal.InternalEditorUtility.OpenFileAtLineExternal(path, -1);
            }
        }
        else if ((j = comment.IndexOf(".cs:", i)) > -1 && (k = comment.IndexOf(']', j + 4)) > -1)
        {
            if (GUILayout.Button("Open CS In Editor"))
            {
                var name = comment.Substring(i, j - i);
                ...
                if (string.IsNullOrEmpty(path))
                    return;
                j += 4;
                var line = int.Parse(comment.Substring(j, k - j));
        ...
        else if ((j = comment.IndexOf(".cs#", i)) > -1 && (k = comment.IndexOf(']', j + 4)) > -1)
        {
            if (GUILayout.Button("Open CS In Editor"))
            {
                ...
                if (string.IsNullOrEmpty(path))
                    return;
                j += 4;
                var line = FindLineInTextFile(path, @"\b" + comment.Substring(j, k - j) + @"\b");
                UnityEditorInternal.InternalEditorUtility.OpenFileAtLineExternal(path, line);
            }
        }
    }

    static string FindAssetPath(string name, string option)
    {
        foreach (var guid in AssetDatabase.FindAssets(name + option))
        {
            var path = AssetDatabase.GUIDToAssetPath(guid);
            // avoiding Path.GetFileNameWithoutExtension(path) ...
    ...
    public delegate void OnScriptableObjectInstanced(ScriptableObject asset);
    ...
    /// </summary>
    /// ***********************************************************************
    public static T CreateAssetOfType<T>(OnScriptableObjectInstanced callBackInstanced = null) where T : ScriptableObject
    {
        T asset = ScriptableObject.CreateInstance<T>();

        callBackInstanced?.Invoke(asset);
    ...
    //// 現在の地域のテキスト表を全て列挙する
    //public static List<string> RefreshTextTableNames()
    //{
    //    List<string> table_names = new List<string>();

    //    string[] files = Directory.GetFiles(Application.dataPath + "/"
    //        + AssetManager.RESOURCE_ASSETBUNDLE_PATH
    //        + "Localize/"
    //        + SystemManager.LanguageCode + "/"
    //        + "Text/",
    //        "*.txt", SearchOption.AllDirectories);

    //    for (int i = files.Length - 1; i >= 0; --i)
    //    {
    //        table_names.Add(Path.GetFileNameWithoutExtension(files[i]));
    //    }
    //    return table_names;
    //}
    ...
    // Assets/Data/ 以下
    public static T LoadFromData<T>(string path) where T : UnityEngine.Object
    {
        var obj = UnityEditor.AssetDatabase.LoadAssetAtPath<T>("Assets/" + "Data/" + path);
        return obj;
    }
    ...
    ///// </summary>
    ///// ***********************************************************************
    //public static string LoadZip(string path, string file_name)
    //{
    //    var str = Zip.ReadFile("Assets/" + AssetManager.RESOURCE_ASSETBUNDLE_PATH + path + file_name + ".bytes", file_name + ".txt");
    //    return str;
    //}
    ...
    /// <summary>null非許容なSerializeFieldがnullになっている場合エラーメッセージを出す</summary>
    ...
    //        var maybeNull = ReflectionUtility.GetCustomAttribute<MaybeNullAttribute>(field);
    //        if (maybeNull != null)
    //            continue;
    ...
    //            if (stage != null)
    //            {
    //                // replace to clickable bahaviour in Debug.Console
    //                var t = stage.prefabContentsRoot.transform.Find(hierarchy);
    //                if (t != null)
    //                {
    //                    var b = t.GetComponent(type) as MonoBehaviour;
    //                    if (b != null)
    //                        behaviour = b;
    //                }
    //            }
    //            var msg = $"null非許容のフィールド[{field.Name}]の値がnullの状態です <{type.Name}> ({hierarchy})";
    //            DebugUtility.LogError(msg, behaviour);
    //            sb.Append(msg);
    //            sb.Append('\n');
    //        }
    //    }
